...
chainingFunction.prototype.ajaxReq =
    function(param) {
        ...
        var xhrFields = param.xhrFields;

        if (!url) { alert("URL not specified");
            return false; }
        if (!success) { alert("Success not specified");
            return false; }
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                return;
            } else if (xhr.readyState == 4) { error(xhr);
                return }
        };
        if (xhrFields) {
            if(xhrFields!= null && typeof(xhrFields) == 'object'){
